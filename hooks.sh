...
function pre_compile {
	...
	if [ ! -f $APPDIR/priv/lngs.csr ]; then
		echo "Certificate Signing Request does not exist, generating..."
		openssl req -new -batch -key $APPDIR/priv/key -out $APPDIR/priv/lngs.csr
		RES=$?
		if [ $RES != 0 ]; then
	...
	if [ ! -f $APPDIR/priv/lngs.crt ]; then
		echo "Certificate does not exists, generating self-signed for a year..."
		openssl x509 -req -days 365 -in $APPDIR/priv/lngs.csr -signkey $APPDIR/priv/key -out $APPDIR/priv/lngs.crt
		RES=$?
		if [ $RES != 0 ]; then
