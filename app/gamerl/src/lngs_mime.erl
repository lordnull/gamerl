...
-export([from_path/2, from_ext/1]).

from_path(Path, _) ->
	Ext = filename:extension(Path),
	case from_ext(Ext) of
		undefined ->
			[{<<"text">>, <<"plain">>, []}];
		BinType ->
			[Major, Minor] = binary:split(BinType, <<$/>>),
			[{Major, Minor, []}]
	end.
...
from_ext(<<".eot">>) -> <<"application/vnd.ms-fontobject">>;
from_ext(<<".m4v">>) -> <<"video/mp4">>;
from_ext(<<".svg">>) -> <<"image/svg+xml">>;
from_ext(<<".svgz">>) -> <<"image/svg+xml">>;
from_ext(<<".ttc">>) -> <<"application/x-font-ttf">>;
from_ext(<<".ttf">>) -> <<"application/x-font-ttf">>;
from_ext(<<".vcf">>) -> <<"text/x-vcard">>;
from_ext(<<".webm">>) -> <<"video/web">>;
from_ext(<<".webp">>) -> <<"image/web">>;
from_ext(<<".woff">>) -> <<"application/x-font-woff">>;
from_ext(<<".otf">>) -> <<"font/opentype">>;
from_ext(<<".json">>) -> <<"application/json">>;
